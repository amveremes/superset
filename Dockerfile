...
FROM apache/superset:6.0.0 AS prod
...
# And packages using uv into the virtual environment
RUN apt-get update && \
    apt-get install -y \
    gcc \
    pkg-config \
    ...
    pymssql \
    # add the driver(s) for your data warehouse(s), in this example we're showing for MySQL:
    mysqlclient \
